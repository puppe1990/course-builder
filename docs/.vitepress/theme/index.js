...
function syncRootLocaleLabel(siteData, href = "") {
  ...
  locales.root.label = relative.startsWith("/zh/")
    ? locales.zh.label
    : rootEnglishLabel;
}
...
function ensureMermaidViewer() {
  ...
  document.addEventListener("keydown", (event) => {
    if (
      event.key === "Escape" &&
      mermaidViewer?.classList.contains("is-open")
    ) {
      closeMermaidViewer();
    } else if (
      (event.key === "+" || event.key === "=") &&
  ...
  mermaidViewerBody?.addEventListener(
    "wheel",
    (event) => {
...
function serializeMermaidSvg(svg) {
  ...
  if (!clone.getAttribute("viewBox")) {
    clone.setAttribute(
      "viewBox",
      `0 0 ${Math.max(1, Math.round(rawWidth))} ${Math.max(1, Math.round(rawHeight))}`,
    );
  }
...
function updateMermaidViewerFitScale() {
  ...
  mermaidViewerFitScale = Math.max(
    0.25,
    Math.min(
      maxWidth / mermaidViewerNaturalWidth,
      maxHeight / mermaidViewerNaturalHeight,
    ),
  );
}
...
function setMermaidViewerScale(nextScale) {
  ...
  const width =
    mermaidViewerNaturalWidth * mermaidViewerFitScale * mermaidViewerScale;
  const height =
    mermaidViewerNaturalHeight * mermaidViewerFitScale * mermaidViewerScale;

  mermaidViewerStage.style.width = `${width}px`;
  mermaidViewerStage.style.height = `${height}px`;

  const resetButton = mermaidViewer?.querySelector(
    '[data-mermaid-action="reset"]',
  );
